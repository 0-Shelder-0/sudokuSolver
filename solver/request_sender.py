...
def send_request_to_create_status(solution_id: int):
    ...
    send_request(f'{BASE_URL}/status/', model)


def send_request_to_update_solution(solution, solution_id):
    model = SolutionUpdate(solution_id, solution)
    send_request(f'{BASE_URL}/solution/', model)


def send_request(web_url, model):
    web_request_body = json.dumps(model)
    headers = {'api-token': API_TOKEN}

    web_response = requests.put(web_url, json=web_request_body, headers=headers, timeout=10)
